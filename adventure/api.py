...
def move(request):
    ...
    if nextRoomID is not None and nextRoomID > 0:
        ...
        #     pusher.trigger(f'p-channel-{p_uuid}', u'broadcast', {'message':f'
        # {player.user.username} has walked {dirs[direction]}.'})
        # for p_uuid in nextPlayerUUIDs:
        #     pusher.trigger(f'p-channel-{p_uuid}', u'broadcast', {'message':f'
        # {player.user.username} has entered from the
        # {reverse_dirs[direction]}.'})
        return JsonResponse({
            'name': player.user.username,
